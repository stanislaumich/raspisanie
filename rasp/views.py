...
from .models import Aud, Para, MyPers, Mess
from .models import Grp
from .models import Person
from .models import Predmet, Rasp
from datetime import timedelta, datetime, date
from .forms import EditRasp, Login, MessAdd
...
def indexPerson(request):
    ...
    people = MyPers.objects.filter(myid=request.session['userid']).all()
    # print(people)
...
def indexAud(request):
    ...
    page_number = request.GET.get("page")
    page_obj = paginator.get_page(page_number)
    return render(request, "rasp/indexAud.html", context={"aud": a, "wd": wd, "page_obj": page_obj})
...
def detailRaspAud(request, id, wd):
    ...
    if not g:
        ...
        return render(request, 'rasp/404aud.html', cntx)
    k = 0
    w = []
    dtb = datefromiso(date.today().year, wd, 1).date()
    dtb = dtb + timedelta(-1 * dtb.weekday() + 0)
    ...
    return render(request, "rasp/detailRaspAud.html",
                  context=cntx)
